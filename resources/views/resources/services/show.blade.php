@extends('layouts.admin')

@section('content')

<div class="card text-bg-theme">

     <div class="card-header d-flex justify-content-between align-items-center p-3">
        <h4 class="m-0">{{ isset($service->name) ? $service->name : 'Услуга' }}</h4>
        <div>
            <form method="POST" action="{!! route('services.destroy', $service->id) !!}" accept-charset="UTF-8">
                <input name="_method" value="DELETE" type="hidden">
                {{ csrf_field() }}

                <a href="{{ route('services.edit', $service->id ) }}" class="btn btn-info" title="Редактировать">
                    <span class="fa-regular fa-pen-to-square" aria-hidden="true"></span>
                </a>

                <button type="submit" class="btn btn-danger" title="Удалить" onclick="return confirm(&quot;Click Ok to delete Service.?&quot;)">
                    <span class="fa-regular fa-trash-can" aria-hidden="true"></span>
                </button>

                <a href="{{ route('services.index') }}" class="btn btn-info" title="Показать все">
                    <span class="fa-solid fa-table-list" aria-hidden="true"></span>
                </a>

                <a href="{{ route('services.create') }}" class="btn btn-secondary" title="Создать">
                    <span class="fa-solid fa-plus" aria-hidden="true"></span>
                </a>

            </form>
        </div>
    </div>

    <div class="card-body">
        <dl class="row">
            <dt class="text-lg-end col-lg-2 col-xl-3">Название</dt>
            <dd class="col-lg-10 col-xl-9">{{ $service->name }}</dd>
            <dt class="text-lg-end col-lg-2 col-xl-3">Цена</dt>
            <dd class="col-lg-10 col-xl-9">{{ $service->price }}</dd>
            <dt class="text-lg-end col-lg-2 col-xl-3">Категория</dt>
            <dd class="col-lg-10 col-xl-9">{{ optional($service->Category)->name }}</dd>
            <dt class="text-lg-end col-lg-2 col-xl-3">Акции</dt>
            <dd class="col-lg-10 col-xl-9">
                <ol>
                    @foreach ($service->discounts as $discount)
                        <li>{{ $discount->header }}</li>
                    @endforeach
                </ol>
            </dd>
            <dt class="text-lg-end col-lg-2 col-xl-3">Дата создания</dt>
            <dd class="col-lg-10 col-xl-9">{{ $service->created_at }}</dd>
            <dt class="text-lg-end col-lg-2 col-xl-3">Дата последнего обновления</dt>
            <dd class="col-lg-10 col-xl-9">{{ $service->updated_at }}</dd>

        </dl>

    </div>
</div>

@endsection